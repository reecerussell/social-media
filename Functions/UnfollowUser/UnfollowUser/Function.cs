...
namespace UnfollowUser
{
    public class Functions
    {
        ...
        public async Task<APIGatewayProxyResponse> UnfollowUser(APIGatewayProxyRequest request, ILambdaContext lambdaContext)
        {
            using var context = new Context(request, true);

            try
            {
                var (success, _, error) = await _userManager.UnfollowAsync(context, context.GetValue<string>("id"));
                if (!success)
                {
